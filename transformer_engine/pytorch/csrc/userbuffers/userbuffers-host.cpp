...
int register_user_buffer_collective(void **gpubuff, size_t bytes, communicator *comm, bool alloc) {
  ...
  if (alloc) {
    ...
    prop.requestedHandleTypes = CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR;//CU_MEM_HANDLE_TYPE_FABRIC;
    ...
    CUCHECK(cuMemExportToShareableHandle(&peerfd[myrank], comm->uchandles[hndl][myrank], CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR, 0 /*flags*/));
    ...
    for(int p=0;p<nranks;p++) {
      if(p!=myrank)
        CUCHECK(cuMemImportFromShareableHandle(&comm->uchandles[hndl][p], (void *)peerfd[p], CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR));
      close(peerfd[p]);
    }
    CUdeviceptr ptr;
    ...
    if(comm->use_mc && comm->mc_maxsize >= comm->mc_offset+aligned_size) {
      CUCHECK(cuMulticastBindMem(comm->mc_handle, comm->mc_offset, comm->uchandles[hndl][myrank], 0 /*memOffset*/, aligned_size, 0));
